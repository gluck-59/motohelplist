...
myApp.onPageInit('messages', function (page) 
{
   ...
   // var loading = false; 
        ...
        if (messagePic && imageType.test(messagePic.type))
        {
            if (window.File && window.FileReader && window.FileList && window.Blob) 
            {
                ...
                { 
                    ...
                    $$('.messagesHolder').append('<canvas id="messagePicCanvas" width="'+width+'" height="'+height+'">'+ lang('browserNotSupported') +'</canvas>');
                ...
                myApp.alert( lang('browserNotSupported') );
            }
        }
    });
    ...
    // отправка в десктопах - и по нажатию кнопки и по энтеру
...
console.log(e.keyCode)            
    ...
    // отправка по нажатию кнопки - для всех
...
function getChannelInfo(id_channel)
{
    ...
    var channelInfo = new Promise(function(resolve, reject) 
    {
        ...
        xhr.onerror = function() {
          ...
          myApp.hidePreloader();
          myApp.alert( lang('tryLater'), lang('noInternet10') );
          return;
        };
    ...
    channelInfo.then(function(response)
    {
        ...
        var channelcount = response.count + ' ' + lang('members');
    ...
    '<div class="pages"><div class="page" data-page="messages" data-id_channel="'+id_channel+'" data-id_from="'+id_from+'">' +
        '<div class="toolbar messagebar">'+
            ...
            '<span class="fileinput-button" data-role="button" data-icon="plus"><input id="chat-attach" type="file" accept="image/*" id="capture"></span>'+
                '<textarea placeholder="answer"></textarea><a href="#" class="link send-message">Send</a>'+
            '</div>' +
        '</div>'+
...
function addMessage()
{
    ...
    $$.ajax(
    {
        ...
        }, error: function(response)
        {
            ...
            {
                myApp.alert( lang('channelNotExist') );
            }
        }
    });        
}