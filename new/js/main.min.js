...
var $$ = Dom7;
var API_URL = '//app.motohelplist.com/api/v1/';
...
if (!myApp.formGetData('settings') || !myApp.formGetData('settings').relativeTime || !myApp.formGetData('settings').sosAirOnly || !myApp.formGetData('settings').animation || !myApp.formGetData('settings').geocodeAddr || !myApp.formGetData('settings').clusterPoi || !myApp.formGetData('settings').showTraffic || !myApp.formGetData('settings').initZoom || !myApp.formGetData('settings').pois || !myApp.formGetData('settings').showTripFriendsOnly || !myApp.formGetData('settings').userLanguage)
{
    myApp.ls.setItem('f7form-settings', JSON.stringify({"relativeTime":"1","sosAirOnly":"0","animation": "1","geocodeAddr":"0","clusterPoi":"11","showTraffic":"0","initZoom":"14","pois":["hotels","services","tireservices","places","parkings"], "showTripFriendsOnly": "0", "userLanguage": "auto"}));
}
...
localStorage.setItem('lastAccuracy', '1970'); 
...
Template7.global = {}; // оставить здесь в таком виде
...
if (!localStorage.getItem('mapLargeScreen') )   localStorage.setItem('mapLargeScreen', '0');
if (!localStorage.getItem('addToHomeScreen') )  localStorage.setItem('addToHomeScreen', '0');
...
var geocoder;
var setCenter = 0;
var mapLargeScreen = localStorage.getItem('mapLargeScreen');
var addToHomeScreen = localStorage.getItem('addToHomeScreen');


if (myApp.formGetData('settings').userLanguage == 'auto')
{
    language = (navigator.language || navigator.systemLanguage || navigator.userLanguage).substr(0, 2).toLowerCase();
    if (language != 'en' && language != 'ru') language = 'en';
}
else language = myApp.formGetData('settings').userLanguage;
...
{
//    myApp.alert(lang('browserNotSupported')); это уже есть в firstPage()
    //myApp.alert('123');
}
...
$$('.form-to-json.login').on('click', function() 
{
    ...
    if (lp)
    {
        ...
        {
            myApp.alert(lang('someThingMissing'));
            return false;
        }
    }
    ...
    $$('#login').addClass('disabled');    
        ...
        $$.ajax(
        {
            ...
            success: function(response)
            {
                ...
                myApp.addNotification({
                    title: lang('returnedUser'),
                    message: lang('returnedUserExtended'),
                    hold: 2000
                });          
                myApp.closeModal();
        ...
        error: function(response)
        {
            ...
            {
                myApp.alert(lang('incorrectPhoneFormat'));    
            }
            ...
            if (response.status == 403)
            {
                myApp.modal(
                {
                    title:  lang('wrongPassword'),
                    text: lang('rememberOrNew'),
                    verticalButtons: true,
                    buttons: [{
                    text: lang('illCallToMem'),
                    onClick: function() 
                    {
                        return;
                    }},
            ...
            if (response.status == 401)
            {
                myApp.modal(
                {
                    title: lang('unknownPhoneNum'),
                    text: lang('firstTimeHere'),
                    verticalButtons: true,
                    buttons: [{
                    text: lang('changePhone'),
                    onClick: function() 
                    {
                        return;
                    }},
...
function initAutocompleteCity() {
        ...
        var autocompleteStandaloneAjax = myApp.autocomplete({
            ...
            limit: 50,
            notFoundText: lang('cityNotFound'),
            searchbarPlaceholderText: lang('latinChar'),
...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    // экспериментально 
    // надоедалка "сохранить иконку на экран"
    if ( ("standalone" in navigator && navigator.standalone != true || !window.matchMedia('(display-mode: standalone)').matches) && addToHomeScreen < 5 )
    {
        localStorage.setItem('addToHomeScreen', (addToHomeScreen*1) + 1);
        setTimeout(function()
        {
            if ( os == 'ios')
            {
                myApp.alert(lang("addToHomeScreenIos"));
            }
            if ( os == 'android')
            {
                myApp.alert(lang("addToHomeScreenAndroid"));
            }
        }, 10000);
    }    
});
...
myApp.onPageInit('bugreport', function (page) 
{
    ...
    var dev = myApp.device;
    $$('textarea[name=debug]').html('<br>'+myApp.formGetData('profile').name+', '+myApp.formGetData('profile').phone+
    '<br>android: '+dev.android+
    '<br>androidChrome: ' + dev.androidChrome+
    '<br>ios: ' + dev.ios+
    '<br>ipad: ' + dev.ipad+
    '<br>iphone: ' + dev.iphone+
    '<br>minimalUi: ' + dev.minimalUi+
    '<br>os: ' +dev.os+
    '<br>osVersion: ' +dev.osVersion+
    '<br>иконка: '+navigator.standalone+'<br><br>');
    ...
    //         url: API_URL+'users/bugreport',
            ...
            complete: 	function (xhr, status) {
                if (status = 200)
                {
                    ...
                    myApp.alert( lang('weTookYouReport'), lang('thank'));
                ...
                else 
                {
                    myApp.hideProgressbar();
                    $$('.button').removeClass('disabled');                        
                    myApp.alert( lang('unableToSendMessage') );
                    setTimeout(function() {myApp.mainView.loadPage('onair.html')}, 500);                
                }
            }
        });
...
myApp.onPageInit('registration', function (page) 
{
    ...
    $$('.form-to-json.registration').on('click', function() 
    {
        ...
        if (reg.id_city == '')
            {
                myApp.alert( lang('chooseCity') );
                return false;
            }
        else if (reg.code == '')
        {
            myApp.alert( lang('smsCodeNeed') );
            return false;            
        }
        else if (reg.agree != '1')
        {
            myApp.alert( lang('termsOfUseNeed') );
            return false;                        
        }
        else if (reg.code.length != 4)
        {
            myApp.alert( lang('wrongSmsCode') );
            return false;            
        }
        else
        {
...
//        $$('#upload').click(); // автоклик не работает в мобильных
    ...
    {
        myApp.confirm( lang('deleteAvatarExtended'), lang('deleteAvatar'), 
            function () 
            {
                $$.ajax(
                {
...
*/
        ...
        $$('.form-to-json.profile-change').on('click', function() 
        {
            $$.ajax(
            {
                ...
                error: function(response)
                {
                    ...
                    myApp.addNotification({
                        title: lang('saveProfileError'),
                        text: lang('pleaseBugreport'),
                        hold: 4000
                    });    
        ...
        var autocompleteStandaloneAjax = myApp.autocomplete({
            ...
            textProperty: 'name', //object's "text" property name
            limit: 50,
            notFoundText: lang('motoNotFound'),
            preloader: true, //enable preloader
            source: function (autocomplete, query, render) {
                var results = [];
                if (query.length === 0) {
...
myApp.onPageInit('settings', function (page) 
{
    ...
    /* сформируем селекты на лету */
    /***/
    // управление языками
    myApp.smartSelectAddOption('[name=userLanguage]', '<option value="auto"'+(language.indexOf('auto') !=-1 ? 'selected': '')+ '>System</option>');
    myApp.smartSelectAddOption('[name=userLanguage]', '<option value="en"'+(language.indexOf('en') !=-1 ? 'selected': '')+ '>English</option>');
    myApp.smartSelectAddOption('[name=userLanguage]', '<option value="ru"'+(language.indexOf('ru') !=-1 ? 'selected': '')+ '>Русский</option>');

    // время событий
    var eventTime = myApp.formGetData('settings').relativeTime;    
    myApp.smartSelectAddOption('[name=relativeTime]', '<option value="0"'+(eventTime.indexOf('0') !=-1 ? 'selected': '')+ '>'+lang('absolute')+'</option>');        
    myApp.smartSelectAddOption('[name=relativeTime]', '<option value="1"'+(eventTime.indexOf('1') !=-1 ? 'selected': '')+ '>'+lang('relative')+'</option>');    
    
    // какие каналы Эфира показывать
    var sosAirOnly = myApp.formGetData('settings').sosAirOnly;
    myApp.smartSelectAddOption('[name=sosAirOnly]', '<option value="0"'+(sosAirOnly.indexOf('0') !=-1 ? 'selected': '')+ '>'+lang('all')+'</option>');
    myApp.smartSelectAddOption('[name=sosAirOnly]', '<option value="1"'+(sosAirOnly.indexOf('1') !=-1 ? 'selected': '')+ '>'+lang('sosOnly')+'</option>');
    
    // анимация интерфейса
    var animation = myApp.formGetData('settings').animation;
    myApp.smartSelectAddOption('[name=animation]', '<option value="0"'+(animation.indexOf('0') !=-1 ? 'selected': '')+ '>'+lang('partial')+'</option>');
    myApp.smartSelectAddOption('[name=animation]', '<option value="1"'+(animation.indexOf('1') !=-1 ? 'selected': '')+ '>'+lang('full')+'</option>');

    // слои карты
    var pois = myApp.formGetData('settings').pois;
    myApp.smartSelectAddOption('[name=pois]', '<option value="hotels"'+(pois.indexOf('hotels') !=-1 ? 'selected': '')+ '>'+lang('hotels')+'</option>');
    myApp.smartSelectAddOption('[name=pois]', '<option value="services"'+(pois.indexOf('services') !=-1 ? 'selected': '')+ '>'+lang('services')+'</option>');
    myApp.smartSelectAddOption('[name=pois]', '<option value="tireservices"'+(pois.indexOf('tireservices') !=-1 ? 'selected': '')+ '>'+lang('tireservices')+'</option>');
    myApp.smartSelectAddOption('[name=pois]', '<option value="places" '+(pois.indexOf('places') !=-1 ? 'selected': '')+ '>'+lang('places')+'</option>');
    myApp.smartSelectAddOption('[name=pois]', '<option value="parkings"'+(pois.indexOf('parkings') !=-1 ? 'selected': '')+ '>'+lang('parkings')+'</option>');

    // кого показывать в пути
    var showTripFriendsOnly = myApp.formGetData('settings').showTripFriendsOnly;
    myApp.smartSelectAddOption('[name=showTripFriendsOnly]', '<option value="0"'+(showTripFriendsOnly.indexOf('0') !=-1 ? 'selected': '')+ '>'+lang('all')+'</option>');
    myApp.smartSelectAddOption('[name=showTripFriendsOnly]', '<option value="1"'+(showTripFriendsOnly.indexOf('1') !=-1 ? 'selected': '')+ '>'+lang('friendsOnly')+'</option>');
    
    // начальный масштаб карты
    var initZoom = myApp.formGetData('settings').initZoom;
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="0"'+(initZoom.indexOf('0') !=-1 ? 'selected': '')+ '>0</option>');    
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="1"'+(initZoom.indexOf('1') !=-1 ? 'selected': '')+ '>1</option>');    
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="2"'+(initZoom.indexOf('2') !=-1 ? 'selected': '')+ '>2</option>');    
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="3"'+(initZoom.indexOf('3') !=-1 ? 'selected': '')+ '>3</option>');
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="4"'+(initZoom.indexOf('4') !=-1 ? 'selected': '')+ '>4</option>');
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="5"'+(initZoom.indexOf('5') !=-1 ? 'selected': '')+ '>5</option>');    
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="6"'+(initZoom.indexOf('6') !=-1 ? 'selected': '')+ '>6</option>');        
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="7"'+(initZoom.indexOf('7') !=-1 ? 'selected': '')+ '>7</option>');        
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="8"'+(initZoom.indexOf('8') !=-1 ? 'selected': '')+ '>8</option>');        
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="9"'+(initZoom.indexOf('9') !=-1 ? 'selected': '')+ '>9</option>');        
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="10"'+(initZoom.indexOf('10') !=-1 ? 'selected': '')+ '>10</option>');        
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="11"'+(initZoom.indexOf('11') !=-1 ? 'selected': '')+ '>11</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="12"'+(initZoom.indexOf('12') !=-1 ? 'selected': '')+ '>12</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="13"'+(initZoom.indexOf('13') !=-1 ? 'selected': '')+ '>13</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="14"'+(initZoom.indexOf('14') !=-1 ? 'selected': '')+ '>14</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="15"'+(initZoom.indexOf('15') !=-1 ? 'selected': '')+ '>15</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="16"'+(initZoom.indexOf('16') !=-1 ? 'selected': '')+ '>16</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="17"'+(initZoom.indexOf('17') !=-1 ? 'selected': '')+ '>17</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="18"'+(initZoom.indexOf('18') !=-1 ? 'selected': '')+ '>18</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="19"'+(initZoom.indexOf('19') !=-1 ? 'selected': '')+ '>19</option>');            
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="20"'+(initZoom.indexOf('20') !=-1 ? 'selected': '')+ '>20</option>');                
    myApp.smartSelectAddOption('[name=initZoom]', '<option value="21"'+(initZoom.indexOf('21') !=-1 ? 'selected': '')+ '>21</option>');                
    
    // кластеризовать POI    
    var clusterPoi = myApp.formGetData('settings').clusterPoi;
    myApp.smartSelectAddOption('[name=clusterPoi]', '<option value="2"'+(clusterPoi.indexOf('2') !=-1 ? 'selected': '')+ '>'+lang('far')+'</option>');
    myApp.smartSelectAddOption('[name=clusterPoi]', '<option value="11"'+(clusterPoi.indexOf('11') !=-1 ? 'selected': '')+ '>'+lang('beside')+'</option>');    
    
    // показывать адреса POI
    var geocodeAddr = myApp.formGetData('settings').geocodeAddr;
    myApp.smartSelectAddOption('[name=geocodeAddr]', '<option value="0"'+(geocodeAddr.indexOf('0') !=-1 ? 'selected': '')+ '>'+lang('no')+'</option>');    
    myApp.smartSelectAddOption('[name=geocodeAddr]', '<option value="1"'+(geocodeAddr.indexOf('1') !=-1 ? 'selected': '')+ '>'+lang('yes')+'</option>');        
    
    // показывать google пробки
    var showTraffic = myApp.formGetData('settings').showTraffic;    
    myApp.smartSelectAddOption('[name=showTraffic]', '<option value="0"'+(showTraffic.indexOf('0') !=-1 ? 'selected': '')+ '>'+lang('no')+'</option>');    
    myApp.smartSelectAddOption('[name=showTraffic]', '<option value="1"'+(showTraffic.indexOf('1') !=-1 ? 'selected': '')+ '>'+lang('yes')+'</option>');        
...
myApp.onPageInit('onair', function (page) 
{
    ...
    $$('#air').on('show', function () {
        ...
        $$('#onairResults').html('');
        digestFill(1,0, itemsPerLoad);
        $$('#navbar-name').text( lang('chatChannels') );
...
$$('input#allowgps').on("change", function()
{
    if (this.checked == true)
    {
        ...
        setTimeout(function()
        {
            ...
            if (!myApp.ls.lastLat || !myApp.ls.lastLng)
            {
                ...
                myApp.addNotification({
                    title: lang('unableToDetermineGeo'),
                    message: lang('maybeDeviceNotFoundSat'),
                    hold: 4000
                });                        
            }
        },1000);
    ...
    {
        $$('#navbar-name').text( lang('findChannel') );
        var autocompleteStandaloneAjax = myApp.autocomplete({
...
myApp.onPageInit('friends', function (page) 
{
    ...
    //        var profile = myApp.formGetData('profile');
            switch (this.dataset.link) 
            {
                ...
                {
                    myApp.alert( lang('profileIsCorrupt') );
                    return;
                }
                ...
                {
                    myApp.alert( lang('cycleNotSpecified') );
                    return;
                }
...
//console.log('map', page.query, page.query.lat, page.query.lng);
    ...
    {
        $$('body').addClass('map');
        // покажем сообщение 2 раза и больше не будем
        if ( mapLargeScreen < 3 )
        {
            localStorage.setItem('mapLargeScreen', (mapLargeScreen*1) + 1); 
            myApp.addNotification({
                title: lang('fullscreenForDesktop'),
                message: lang('areYouNotAgainst'),
                hold: 4000
            });        
        }
    } 
    ...
    $$('form.add_poi').on('submit', function (e) 
    {
        ...
        $$.ajax(
        {
            ...
            error: function(response)
            {
                ...
                myApp.closeModal();       
                myApp.addNotification({
                    title: lang('noInternet'),
                    message: lang('tryLater'),
                    hold: 4000
                });
            }    
        });
...
myApp.onPageInit('help_list', function (page) 
{
    ...
    myApp.addNotification({
        title: lang('geoNotSupported'),
        message: lang('distCaclNotPossible'),
        hold: 4000
    });
	}
        ...
        $$('.swipeout').on('delete', function () 
        {
            ...
            $$.ajax(
            {
                ...
                success: function(response)
                {
                    ...
                    {
                        myApp.addNotification({
                                title: lang('userDeletedFromChannel'),
                                hold: 2000
                            });
                    }
                    else
...
//    loadWorker = null;
    ...
    myApp.ls.setItem('helplist_updated', '0');    
...
//    var patternhttp = /([-a-zA-Z0-9@:%_\+.~#?&\/\/=]{2,256}\.[a-z]{2,4}\b(\/?[-a-zA-Z0-9@:%_\+.~#?&\/\/=]*)?)/gi; // оригинал
    ...
    else if (pattern.test(inputText))
    {
        ...
        return inputText.replace(patterngps, '<br><a href="map.php?lat=$1&lng=$2" role="external">'+ lang('gpsCoords') +'</a>');
    }
    else
    return inputText;
}
...
*/    
    ...
    profile.then(function(response)
    {
        ...
        // если юзер в пути, подтянем его точку
        var userLat;
        var userLng;
        db.transaction(function (tx) 
        {
            tx.executeSql("select lat, lng from trips where id_user = "+userprofile[0].id_user,[],function(tx, results)
            {
                    if (results.rows.length > 0)
                    {
                        for (var i = 0; i < results.rows.length; i++) 
                        { 
                            userLat = ( results.rows.item(i).lat);
                            userLng = ( results.rows.item(i).lng );
                        }
                    }
            }, function (tx, error) 
            { 
       ...
       // сформируем кнопки - написать, позвонить итд
        var buttons1 = [{text: lang('write'), onClick: function () { getChannelMessages(0, userprofile[0].name, id_user) }}];
        
        if ( parseInt(userprofile[0].active) == 1 && parseInt(myApp.formGetData('profile').active) == 1 )
            buttons1.push({ text: '<a href="tel:+'+userprofile[0].phone+'" class="external">'+ lang('call') +'</a>'} );

        // дадим время сходить в базу за трипом
        // и пропушим кнопку если трип есть
        setTimeout(function()
        {
        if (userLat && userLng)
            buttons1.push(
                { 
                    ...
                    onClick: function()
                    {
                        myApp.mainView.loadPage('map.html?lat='+userLat+'&lng='+userLng);
                    }
                } );
        },1000);
        ...
        if ( userprofile[0].is_friend )
            var buttons2 = [{text: lang('unFriend'),onClick: function () 
                { 
                    unfriend(id_user); 
                    //mainView.router.back() 
                }}];
        else
            var buttons2 = [{text: lang('toFriend'), onClick: function () 
                { 
                    tofriend(id_user); 
                    //mainView.router.back() 
                }}];

        var buttons4 = [{text: lang('Cancel'), color: 'red' }];
        
        var friends_callto = [buttons1, buttons2, buttons4];
        ...
        $$('#callto').on("click", function() 
        {
            ...
            // если это я — спрашиваем 
                ...
                myApp.confirm( lang('editProfile')+'?', function () {myApp.mainView.loadPage('profile.html')});
        });
...
function updateBadges()
{
    ...
    db.transaction(function (tx) 
    {
        tx.executeSql("SELECT *, private+channels as total from (select IFNULL(sum(unread),0) as private from messages where id_channel = 0) left join (SELECT IFNULL(sum(unread),0) as channels from messages where id_channel >0)",[],function(tx, results)
        {
            ...
            {
                if (navigator.vibrate) navigator.vibrate([200, 100, 200]); // Android 
                blumm.play(); // iPhone            
                //console.warn('BLUMM');
            }
...
/***************************/
//! хелп-лист воркер -- экспериментально перенесен сюда
// вызов внизу через hltick
...
helplist_update_worker.addEventListener('message', function(e) 
{
    ...
    updateHelplist(JSON.parse(e.data).total, JSON.parse(e.data).users, JSON.parse(e.data).to_delete);
}, false);
            ...
            $$.each(results, function(i) 
            {
                ...
                switch (e.data.url) 
                {
                    ...
                    case 'messages/': 
                        ...
                        $$.each(results[i], function(h) 
                        {
                            ...
                            if (myApp.getCurrentView().activePage.name=='onair' && i=="channels") 
                            {
                                try 
                                {
                                    ...
                                    'type_channel' : results[i][h].type_channel,
                                    'urgency' : results[i][h].urgency,
                        ...
                        {
                            if (!myApp.device.os)  // десктопы
                                if (myApp.ls.loadWorkerInterval <= 20000)
                                    myApp.ls.loadWorkerInterval = myApp.ls.loadWorkerInterval * 1 + 500;

                            // для мобил период пока останется 1000                                     
/*
                            else // мобилы
                                if (myApp.ls.loadWorkerInterval <= 2000)
                                    myApp.ls.loadWorkerInterval = myApp.ls.loadWorkerInterval * 1 + 500;                            
*/
                        }
                        /*****************************************/
...
db.transaction(function (tx) 
{
    ...
    function(tx, results) {
       //console.log('delete trips (обновление)',results.rowsAffected);
        ...
        // изменяемое время работы Helplist loadWorker
        var hlTimerId = setTimeout(function hltick() 
        {
            helplist_update_worker.postMessage({"token": myApp.ls.getItem('token'), "lastupdate": lastupdate});
            hlTimerId = setTimeout(hltick, (myApp.ls.loadWorkerInterval*20));
            //console.log('hltick');    
        }, 1000);        
    }
}
...
function firstPage(page)
{
    ...
    setTimeout(function()
    {
        ...
        if (!window.openDatabase || !window.Worker || !navigator.geolocation)
        {
            myApp.alert( lang('browserNotSupported') );
        }
        
        // если нет интернета, показывает алерт и переходит в хелп-лист
/*
        if (!navigator.online)
        {
            myApp.alert('Хелп-листом можно пользоваться даже когда нет связи с интернет.','Нет интернета');
            myApp.mainView.loadPage('help_list.html');
        }
*/
...
window.addEventListener('load', function (e) {
    window.applicationCache.addEventListener('updateready', function (e) {
        if (window.applicationCache.status === window.applicationCache.UPDATEREADY) 
        {
            myApp.modal(
            {
                title:  lang('updateClient'),
                text: lang('updateClientExt'), 
                buttons: 
                [{
                    text: lang('later'),
                    onClick: function () {
...
{    
    // перевод html-шаблонов
    var elems = document.querySelectorAll('[data-lang]');
    for (var i = elems.length; i--;) 
    {
        elems[i].innerHTML = lang(elems[i].getAttribute('data-lang'));
        elems[i].removeAttribute('data-lang');
    }
    ...
    {
        placeholders[i].setAttribute("placeholder", lang(placeholders[i].getAttribute('placeholder')));
    }    
}

function lang(key) 
{
    var str = Template7.global[key] || key;
