function initialise(lat, lng) 
{
    ...
    if (get == 1)
        ...
        latlng = new google.maps.LatLng(parseFloat(myApp.ls.lastLat), parseFloat(myApp.ls.lastLng)); // бабруйска при первой загрузке уже не должно быть
    ...
    {
        // экспериментально не передаем latlng чтобы взять новые    
...
//    if (document.getElementById('submitgeocoder'))
    {
        ...
        document.getElementById('submitgeocoder').addEventListener('submit', function() 
        {
...
//            markerid: pois[i].map_icon+pois[i].id,
        ...
        { 
            ...
            var accuracy_alert = '<div><a href="#" data-popup=".accuracy-alert" class="open-popup">' + lang('accuracyAlert') + '</a></div>';
            if (pois[i].accuracy < 100)
                accuracy_alert = '';
            ...
            if ( (myIdUser < 3 || parseInt(pois[i].owner) == myIdUser) && !pois[i].accuracy)
                var editing = '<div class="right"><a class="updatepoi" data-idpoi="'+pois[i].id+'" data-typepoi="'+pois[i].map_icon+'">'+lang("edit")+'</a></div>';
            else var editing = '';


            var profile = (pois[i].id_user ? '<div class="right"><a href="#" class="external" onclick="getProfile('+pois[i].id_user+')" >&nbsp;'+lang("profile")+'</a></div>' : '');

            var html = (pois[i].price ? pois[i].price + '&nbsp;' + lang("perDay")+'<br>' : '')
            +(pois[i].city ? ''+pois[i].city : '') 
            +(pois[i].accuracy ? '<br>' + lang("accuracy") + ' ' + pois[i].accuracy+'m' : '') 
            +(pois[i].phone1 ? lang('call') + ':&nbsp;<a href="tel:+'+pois[i].phone1+'" class="external">+'+pois[i].phone1+'</a> ' : '')
            +(pois[i].phone2 ? '&nbsp;&nbsp;<a href="tel:+'+pois[i].phone2+'" class="external">+'+pois[i].phone2+'</a>' : '')
            +(pois[i].phone1 || pois[i].phone2 ? '<br>' : '' )
            +(pois[i].parking == 1 ? '<span class="blue tip">'+lang("innerParkingLong")+'</span>' : '')      
            +(pois[i].ac == 1 ? '<span class="blue tip">'+lang("ac")+'</span>' : '')
            +(pois[i].wifi == 1 ? '<span class="blue tip">Wi-Fi</span>' : '')
            +(pois[i].sauna == 1 ? '<span class="blue tip">'+lang("sauna")+'</span>' : '')
            +(pois[i].access == 1 ? '<span class="blue tip">'+lang("access247")+'</span>' : '')
            +(pois[i].camera == 1 ? '<span class="blue tip">'+lang("cameraLong")+'</span>' : '')
            +(pois[i].security == 1 ? '<span class="blue tip">'+lang("security")+'</span>' : '')
            +(pois[i].big == 1 ? '<span class="blue tip">'+lang("bigVolume")+'</span>' : '')
            +(pois[i].electric == 1 ? '<span class="blue tip">'+lang("electric")+'</span>' : '')
            +(pois[i].weld == 1 ? '<span class="blue tip">'+lang("weld")+'</span>' : '')            
            +(pois[i].stock == 1 ? '<span class="blue tip">'+lang("consumables")+'</span>' : '')                        
            +(pois[i].tuning == 1 ? '<span class="blue tip">'+lang("tuning")+'</span>' : '')                                    
            +(pois[i].renewal == 1 ? '<span class="blue tip">'+lang("renewal")+'</span>' : '')
            +(pois[i].germans == 1 ? '<span class="blue tip">'+lang("germans")+'</span>' : '')            
            +(pois[i].japanese == 1 ? '<span class="blue tip">'+lang("japanese")+'</span>' : '')
            +(pois[i].chinese == 1 ? '<span class="blue tip">'+lang("chinese")+'</span>' : '')            
            +(pois[i].podkat == 1 ? '<span class="blue tip">'+lang("podkat")+'</span>' : '')            
            +(pois[i].balancer == 1 ? '<span class="blue tip">'+lang("balancer")+'</span>' : '')            
            +(pois[i].rims == 1 ? '<span class="blue tip">'+lang("rims")+'</span>' : '')                        
            +(pois[i].tire_repair == 1 ? '<span class="blue tip">'+lang("tire_repair")+'</span>' : '')                                    
            +(pois[i].description ? '<br>'+pois[i].description : '')
            +(pois[i].id_user == myIdUser ? accuracy_alert : '')
            +'<hr><div class="small right"><span>'+lang("copyCoords")+'</span>&nbsp;<input class="coords" readonly value="'+pois[i].lat+','+pois[i].lng+'"></div>'
            ; 
            ...
            if (geocodeAddr == 1)
            {
                ...
                setTimeout(function(){ infoWindow.setContent(editing+profile+html1+ (address ? '<br>'+address+'<br>' : '') +html); doMoment(); },500); 
            }
                ...
                $$.ajax({
                    ...
                    { 
                        ...
                        if (myApp.formGetData('profile').active == '1') 
                            var tel = '<br>'+ lang('call') +':&nbsp;<a href="tel:+'+user[0].phone+'" class="external" >+'+user[0].phone+'</a>';
                        else var tel = '';
...
{    
    // экспериментально если не передано latlng, забираем свежие из LS
    if (!latlng)
    ...
    (new google.maps.Geocoder()).geocode({latLng: latlng}, function(resp) 
    {
        ...
        { 
            var accuracy_alert = '<div><a href="#" data-popup=".accuracy-alert" class="open-popup">' + lang('accuracyAlert') + '</a></div>';
            if (myApp.ls.lastAccuracy < 100)
                accuracy_alert = '';
                
            infoWindow.setContent(imhere + resp[0].formatted_address + '<br>' + lang("accuracy") + ': ' + myApp.ls.lastAccuracy + 'm' + accuracy_alert + '<hr><div class="small right">'+ lang("copyCoords") +': <input readonly value="'+latlng.lat().toFixed(6)+','+latlng.lng().toFixed(6)+'"></div>'); 
            infoWindow.open(map, marker); 
        }); 
    });    
}
...
break;
      ...
      case err.PERMISSION_DENINED:
        msg = lang("PERMISSION_DENINED");
        break;
      case err.POSITION_UNAVAILABLE:
        msg = lang("POSITION_UNAVAILABLE");
        break;
      case err.BREAK:
        msg = lang("BREAK");
        break;
      default:
        msg = lang("defaultGeoError");
    }
...
function positionSuccess(position) 
{
    ...
    localStorage.setItem('lastAccuracy', coords.accuracy);
...
//console.log('edit', typeof(edit),edit)
    ...
    google.maps.event.addListener(baseMarker, 'dragend', function (coord) 
    {
        ...
        $$('input[name=latlng]').val('['+lang("change")+'] ' + lat.toFixed(6)+', '+lng.toFixed(6));
