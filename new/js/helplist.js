...
/*
                myApp.addNotification({
                    ...
                    message: 'Измени параметры фильтра или обнови хелп-лист вручную в Настройках',
                    hold: 8000
                });
*/
            myApp.modal(
            {
                title:  'Никого не найдено',
                text: 'Измени параметры фильтра или обнови хелп-лист', 
                buttons: 
                [{
                    text: 'Изменить',
                    onClick: function () {
...
function updateHelplist(total_srv, data, to_delete)
{
    ...
    // сравним сколько юзерей в локальном и серверном хелплистах
    // если колво различается, вызовем updateHelplistManually() и синхронизируем хелписты принудительно
    db.transaction(function (tx) 
    {
        tx.executeSql("select count(id_user) as local_total from helplist",[],function(tx, results)
        {
            myApp.ls.helplist_local_total = results.rows.item(0).local_total;        
            if (myApp.ls.helplist_local_total != total_srv)
            {
                console.log('в локальном хелплисте', myApp.ls.helplist_local_total, 'на сервере', total_srv);
                console.log('обновляем хелплист принудительно');
    ...
    if (navigator.onLine)
    {
        ...
        if (!!window.Worker && !!window.openDatabase && navigator.onLine)
        {
            ...
            helplist_worker_man.addEventListener('message', function(e) 
            {
                ...
                if (qty > 0)
                {
                    updateHelplist(JSON.parse(e.data).total, JSON.parse(e.data).users);
                    //console.log('updateHelplistManually: в updateHelplist передано '+qty);
                    myApp.addNotification({
                        title: 'Готово',
                        message: 'Хелп-лист обновлен',
                        hold: 4000
                    });
                }


//myApp.ls.helplist_local_total = 0;
                ...
                $$('#helplist_local_total').text(myApp.ls.helplist_local_total);
