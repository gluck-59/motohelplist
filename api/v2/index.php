...
$f3->set('DB', new \DB\SQL('mysql:host=localhost;port=3306;dbname=motokofr_motohelplist','motokofr_dbuser','Ji)ouDR2K!1}',array(
...
function($f3,$params) 
{
    ...
    try 
    {
        ...
        else echo json_encode(array("error" => 'Ошибка записи файла'));
...
$f3->route('POST /channels/add',
	function($f3,$params) 
	{
        ...
        $f3->get('DB')->exec('INSERT LOW_PRIORITY INTO webchat_channels  (type_channel, id_object, name, id_urgency) values (:type_channel, :id_object, :name, :id_urgency)',array(':type_channel' => $data['type_channel'], ':id_object' => $id, ':name' => $data['name_channel'], ':id_urgency' => intval($data['urgency'])));
        ...
        if ($data['urgency'] == 0) $string = $data['username'].' создал чат-канал';
        if ($data['urgency'] > 0) $string = $data['username'].' запрашивает помощь';
        if ($data['allowgps'] == 1) 
        {
...
$f3->route('GET /motorcycles/search/@name',
	function($f3,$params) 
	{
        ...
        or manufacturer LIKE ('%{$params['name']}%') 
...
$f3->route(array('GET /helplist/@lastupdate'),
    ...
    $response =  array('count' => count($response), 'to_delete' => $delete, 'users' => $response);
    echo json_encode($response);
    }, 0 // кеширование в сек, больше нельзя
);
...
$f3->route(array('DELETE /profile/avatar'),
    ...
    if (file_exists('../../img/avatar/'.$id.'.jpg'))
    {
        ...
        echo json_encode('У тебя нет аватара');
});
