...
function apiRequest($method, $parameters) {
  if (!is_string($method)) {
    global $date;
    error_log("{$date} __LINE__ Method name must be a string\n", 3, "/1test.log");
    return false;
  }
  ...
  if (!$parameters) {
    ...
    global $date;      
    error_log("{$date} __LINE__ Parameters must be an array\n", 3, "/1test.log");
    return false;
  }
...
error_log("{$date} __LINE__ apiRequest {$url} \n", 3, "/1test.log");
...
error_log(time() .$get." <-- ".print_r($content, true). "\n\n", 3, "1test.log");
