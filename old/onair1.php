...
if ($_SESSION['id_user'])
{
    ...
    if ($_GET['task'] == 'find_channel')
    {
        ...
        {
            $stmt = $pdo->query('SELECT wc.type_channel, wc.id_channel, CONCAT_WS(" ",mc.manufacturer, mc.name) 
            as channel_name,  IF(ws.id_user, 1, 0) as is_subscribe 
            FROM webchat_channels wc
...
left join webchat_subscribe ws on ws.id_channel = wc.id_channel AND ws.id_user = '.$_SESSION['id_user'].'
    ...
    if ($_POST['task'] == 'add_channel' AND $_POST['type_channel'] == 'group')
    {
        ...
        if (intval($_POST['urgency']) == 0)
            $text = getUserName($_SESSION['id_user']).' создал общедоступный групповой канал';
        else            
    ...
    $stmt = $pdo->query('
    SELECT DISTINCT wc.type_channel, wl.id_channel, wl.text, UNIX_TIMESTAMP(wl.ts) as ts, users.name, users.id_user, users.phone, users.gender, cities.name as city, CONCAT_WS(" ",mc.manufacturer, mc.name) as motorcycle, IF(ws.id_channel, 1, 0) as is_subscribe, wc.id_urgency, wu.name_'.$lang.' as urgency, wl.id as lineid,
/*wcu.unread_count,*/
    CASE
    	WHEN wc.type_channel = 1 THEN (SELECT name from cities WHERE id_city = wc.id_object)
    	WHEN wc.type_channel = 2 THEN (SELECT CONCAT_WS(" ",mc.manufacturer, mc.name) FROM motorcycles mc WHERE mc.id_motorcycle = wc.id_object)
