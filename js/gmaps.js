...
/*
// присвоим ссылкам на карте класс external
...
этот способ не работает, используем костыль ниже
*/
    $$('div#map').on('click', function()
    { 
        setTimeout(function()
        {
            $$('div.view-link a').addClass('external');
        }, 2000);
    })
...
function geocodeAddress(geocoder, resultsMap) 
{
    ...
    geocoder.geocode({'address': address}, function(results, status) 
    {
        if (status === google.maps.GeocoderStatus.OK) 
        {
            ...
            placeMarkerImHere(results[0].geometry.location);
        } else {
...
break;
    ...
    setTimeout(function() { 
        ...
        if (google && myApp.ls.lastLat && myApp.ls.lastLng)
            latlng = new google.maps.LatLng(parseFloat(myApp.ls.lastLat), parseFloat(myApp.ls.lastLng));
            
        else
            latlng = new google.maps.LatLng(parseFloat(myApp.ls.initLat), parseFloat(myApp.ls.initLng));
         
        var zoom = parseInt(myApp.formGetData('settings').initZoom);
        if (isNaN(zoom)) var zoom = 14;

// вроде это все надо, но генерит ошибку
map.setCenter(latlng); 
map.setZoom(zoom);
